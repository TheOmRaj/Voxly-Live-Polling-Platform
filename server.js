...
app.post('/api/polls/:id/responses', optionalAuth, async (req, res) => {
  try {
    ...
    if (poll.mode === 'authenticated' && req.user && await Responses.hasResponded(poll.id, req.user.id)) return res.status(409).json({ error: 'Already responded' });
    ...
    const isAnon = poll.mode === 'anonymous' || poll.mode === 'both';
